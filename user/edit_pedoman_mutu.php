...
if(isset($_GET['usulan'])){
	...
	$sql ="SELECT * FROM pedoman_mutu where id = '$userID'";
	...
	foreach ($data as $key) {
		$autoid = $key['id'];
		$kode_prosedur = $key['kode_prosedur'];
		$judul = $key['judul'];
		$file = $key['file'];
		$i++;
	}	
...
<?php
	...
	get_header();
	get_sidebar();
	get_bread_pedoman_mutu();
?>
			<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon white edit"></i><span class="break"></span>Update Pedoman Mutu</h2>
						<div class="box-icon">
							<a href="pedoman_mutu.php" class="btn-close"><i class="halflings-icon white remove"></i></a>
						</div>
					</div>
					<div class="box-content">
						<form class="form-horizontal" method="post" action="update_pedoman_mutu.php" enctype="multipart/form-data">
							<fieldset>
							  <div class="control-group">
								<label class="control-label" for="focusedInput">Kode Prosedur:</label>
								<div class="controls">
								  <input class="input-xlarge focused" name="txtkode" id="focusedInput" type="text" placeholder="Kode Prosedur" 
								  value="<?php echo $kode_prosedur; ?>">
								</div>
							  </div>
							  <div class="control-group">
								<label class="control-label" for="focusedInput">Judul:</label>
								<div class="controls">
								  <input class="input-xlarge focused" name="txtjudul" id="focusedInput" type="text" placeholder="Judul"
