...
if($_FILES['file']['name'] == ""){
	...
	$data = mysqli_query($koneksi, "UPDATE form_mutu SET kode='$kd', judul='$jdl', id_pk='$pd' WHERE id='$autoid'");
	...
	$result = mysqli_query($koneksi, "SELECT * from form_mutu WHERE id=$autoid");

	foreach($result as $r){
		$history = mysqli_query($koneksi, "SELECT * from history WHERE id_dokumen='".$r['id']."' AND id_kategori='4'");
		$tahun = date("Y");
		if(mysqli_num_rows($history) == 0){ // jika history belum ada	
			...
			if(rename($source, $destination)){
				$send = mysqli_query($koneksi, "INSERT INTO history VALUES(NULL, '$tahun', '$tmp_file', '4', '$tmp_id')");
				if (mysqli_num_rows($send) == 0)
				{
			        if(move_uploaded_file($lokasi_file,$folder)){
					$data = mysqli_query($koneksi, "UPDATE form_mutu SET kode='$kd', judul='$jdl', file='$nama_file', id_pk='$pd' WHERE id='$autoid'");
			...
			foreach ($history as $hasil) {
				if($hasil['tahun']==date("Y")){
					if(unlink("../files/history/".$hasil['file'])){
						...
						$destination = "../files/history/$tmp_file";
						$tmp_id = $r['id'];
						$search_id_history = mysqli_query($koneksi, "SELECT * from history where id_kategori='4' and id_dokumen='$tmp_id'");
